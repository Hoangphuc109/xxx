...
const updateinvoice = async (idInvoice, total, purchaseDate, payment, state, isDone, userName, email, phoneNumber, address, users_id, nameProduct, quanity, price, urlImage) => {
    let[results2, fields2] = await connection.query('UPDATE `rice_4_man`.`invoicedetails` SET `nameProduct` = ?, `quanity` = ?, `price` = ?, `urlImage` = ? WHERE `Invoice_idInvoice` = ?;',[nameProduct, quanity, price, urlImage, idInvoice])
